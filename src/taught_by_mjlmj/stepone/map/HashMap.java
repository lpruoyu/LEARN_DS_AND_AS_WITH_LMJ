...
public class HashMap<K, V> implements Map<K, V> {
    ...
    public V put(K key, V value) {
        ...
        if (null == root) {
            root = createNewNode(key, value, null);
            table[index] = root;
            size++;
            afterPut(root);
            return null;
    ...
    public boolean containsValue(V value) {
        ...
        Queue<Node<K, V>> queue = new LinkedListQueue<>();
        for (int i = 0; i < table.length; i++) {
    ...
    private V put(Node<K, V> root, K key, V value) {
        ...
        do {
            ...
            if (h1 > h2) {
                ...
                cmp = -1;
            } else if (Objects.equals(k1, k2)) cmp = 0;
...
//            }
        ...
        Node<K, V> newNode = createNewNode(key, value, parent);
        if (cmp > 0) {
        ...
        while (node != null) {
            ...
            if (h1 > h2) {
                ...
                node = node.left;
                    ...
                    && k1.getClass() == k2.getClass() && k1 instanceof Comparable
                    && (cmp = ((Comparable) k1).compareTo(k2)) != 0
            ) {
...
//            }
    ...
    // 扩容
    private void resize() {
        if (size / ((float) table.length) <= DEFAULT_LOAD_FACTOR) return;
